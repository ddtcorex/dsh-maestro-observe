# Contributing to dsh-maestro-observe

Thank you for contributing to **dsh-maestro-observe** (`@ddtcorex/dsh-maestro-observe`) — trace/health/cost debug tooling for DeepSeek Harness, packaged as a Cordis plugin.

## Getting Started

1. **Fork and clone** `github.com/ddtcorex/dsh-maestro-observe`.
...
4. Open the project in your editor. Host logic lives in `src/host/`, client dashboard in `client/`, tests in `tests/`. `lib/` is build output (gitignored) — never hand-edit it; run `pnpm build` after a source change.
...
- **Scope:** optional, without the `dsh-maestro-` prefix — e.g. `feat(observe):`, `fix(health):`, `docs(readme):`
...
pnpm test        # vitest run (4 files, 23 tests)
pnpm build       # tsc -p tsconfig.json -> lib/
test -f lib/index.js && echo "build ok"
```

After touching the client bundle, verify on live DSH Web (`:3080`), not just curl/grep. Cost keyed by record `ts` — the day aggregate must key by the record's timestamp, not `Date.now()`.
...
- Security vulnerabilities: use GitHub's private advisory reporting at `https://github.com/ddtcorex/dsh-maestro-observe/security/advisories` — do not file a public issue.
